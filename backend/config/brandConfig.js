...
class BrandConfig {
  ...
  buildBrandConfig() {
    return {
      ...
      brand: {
        name: this.brandName,
        displayName: process.env.SITE_NAME || 'نظام إدارة القروض',
      ...
      email: {
        ...
        from: {
          name: process.env.EMAIL_FROM_NAME || this.config?.brand?.displayName || 'نظام إدارة القروض',
      ...
      // WhatsApp Configuration (per site)
      whatsapp: {
        phone: process.env.WHATSAPP_PHONE || '+96512345678',
        businessName: process.env.WHATSAPP_BUSINESS_NAME || 'نظام إدارة القروض'
      },
