// Generate complete user import SQL script with ALL users + admin employee accounts
const fs = require('fs');

// Complete user data from JSON (including missing IDs 35, 36)
const users = [
  ...
  // Adding missing IDs 35 and 36 to complete the sequence
...
-- Generated automatically with ALL 112 users + Admin employee accounts
...
-- 4. Add employee accounts for admins (IDs 3 and 4)
-- Admin 1 as employee (ALI A M ALHABABI)
...
VALUES ('ALI A M ALHABABI (حساب عضو)', '88888888', 'ali.employee@example.com', '${passwordHash}', 'عضو', '88888888', 'employee', 5000.00, '2024-01-01', 'approved', 0, 2, NOW(), NOW());

-- Admin 2 as employee (أحمد الحبابي) 
INSERT INTO users (Aname, phone, email, password, workplace, whatsapp, user_type, balance, registration_date, joining_fee_approved, is_blocked, approved_by_admin_id, created_at, updated_at) 
VALUES ('أحمد الحبابي (حساب عضو)', '88888887', 'ahmed.employee@example.com', '${passwordHash}', 'عضو', '88888887', 'employee', 5000.00, '2024-01-01', 'approved', 0, 2, NOW(), NOW());

-- 5. Import all 112 regular users (starting from ID 5)
`;

// Generate user inserts - now starting from ID 5
users.forEach((user, index) => {
  const newUserId = index + 5; // Start from ID 5 (after 2 admins + 2 admin employee accounts)
...
// Generate joining fee transactions for admin employee accounts
sql += `-- 6. Create joining fee transactions for admin employee accounts
INSERT INTO transaction (user_id, debit, credit, memo, status, transaction_type, date, admin_id, created_at, updated_at)
VALUES (3, 0.00, 10.00, 'رسوم انضمام - حساب إداري', 'accepted', 'joining_fee', '2024-01-01 09:00:00', 2, NOW(), NOW());

INSERT INTO transaction (user_id, debit, credit, memo, status, transaction_type, date, admin_id, created_at, updated_at)
VALUES (4, 0.00, 10.00, 'رسوم انضمام - حساب إداري', 'accepted', 'joining_fee', '2024-01-01 09:00:00', 2, NOW(), NOW());

-- 7. Create joining fee transactions for users who paid 10 KWD
`;

users.forEach((user, index) => {
  if (user["Mempership Fee"] === "10") {
    const newUserId = index + 5;
...
sql += `-- 8. Create active loans for users who have loans > 0
`;
...
SELECT 'ADMIN EMPLOYEE ACCOUNTS' as info;
SELECT user_id, Aname, balance FROM users WHERE user_id IN (3, 4) ORDER BY user_id;
...
SELECT 'SAMPLE REGULAR USERS (First 10)' as info;
SELECT user_id, Aname, balance, joining_fee_approved FROM users WHERE user_type = 'employee' AND user_id >= 5 ORDER BY user_id LIMIT 10;

SELECT 'USER ID RANGES' as info;
SELECT 
  'Admin accounts' as account_type, 
...
UNION ALL
SELECT 
  'Employee accounts' as account_type, 
  MIN(user_id) as min_id, 
  MAX(user_id) as max_id,
  COUNT(*) as total
...
// Write the SQL file
fs.writeFileSync('/root/Loan-Management-System/corrected_complete_import.sql', sql, 'utf8');
console.log('Corrected complete import SQL script generated successfully!');
console.log(`Total users to import: ${users.length} (112 users)`);
...
console.log(`Total accounts (including admin employee accounts): ${users.length + 4} (116 total)`);